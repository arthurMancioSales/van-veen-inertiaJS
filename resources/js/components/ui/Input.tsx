...
const InputField = React.forwardRef<HTMLInputElement, InputType>(
    (
        { className, type, textarea = false, icon: Icon, label, ...props },
        ref,
    ) => {
        ...
        return (
            <div className="flex h-full flex-col">
                <div className="relative h-full">
                    ...
                    {type === "password" && (
                        <Button
                            className="absolute right-2 top-0 hover:bg-transparent hover:text-primary"
                            variant={"ghost"}
                            size={"icon"}
...
const InputError = React.forwardRef<HTMLSpanElement, InputErrorProps>(
    ({ className, error, touched, ...props }, ref) => {
        ...
        if (!error || !touched) return <div className="min-h-[1.5rem]" />;
        return (
            <span
